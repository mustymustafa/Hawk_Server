...
console.log("hirer:" + hirer)
  ...
  static async logRequests(request:Request, response:Response) {
    ...
    const job = await Schema.Job().find({category: 'log'}).where({$or:[{city: city === undefined ? '' : city.trim()},{ city: city2 === undefined ? '' : city2.trim() }, { city2: city === undefined ? '' : city.trim() }, { city2: city2 === undefined ? '' : city2.trim() }]}).and([{status: 'active'}]).sort({'_id': -1})   
